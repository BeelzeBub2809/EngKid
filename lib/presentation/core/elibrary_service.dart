...
class ElibraryService extends GetxService {
  ...
  ElibraryService({required this.appUseCases});
  final UserService _userService = Get.find<UserService>();
  final _preferencesManager = getIt.get<SharedPreferencesManager>();
  final NetworkService _networkService = Get.find<NetworkService>();

  final RxList<Topic> _categoryList = RxList<Topic>.empty(growable: true);
  ...
  final RxList<Elibrary> _bookList = RxList<Elibrary>.empty(growable: true);
  final RxList<Elibrary> _allBookList = RxList<Elibrary>.empty(growable: true);
  final RxInt _totalBook = 0.obs;
  final RxInt _completedBook = 0.obs;

  List<Topic> get categoryList => _categoryList;
  ...
  List<Elibrary> get bookList => _bookList;
  List<Elibrary> get allBookList => _allBookList;
  final RxList<Elibrary> downloadAllReadingList =
      RxList<Elibrary>.empty(growable: true);
  ...
  Future<dynamic> getElibraryCategory() async {
    ...
    //call api get list of categories
    ...
    _categoryList
        .addAll(categories.map((category) => Topic.fromJson(category)));
    }
  ...
  Future<dynamic> onChangeCategory(int index, int categoryId) async {
    if (_networkService.networkConnection.value) {
      ...
      _totalBook.value = data['total_books'] as int;
      _completedBook.value = data['completed_books'] as int;
      _bookList.addAll(books.map((book) => Elibrary.fromJson(book)));
      _bookIndex.value = 0;
        } else {
  ...
  Future<dynamic> getAllElibraryBooks() async {
    ...
    final List<dynamic> books = data['books'] as List<dynamic>;
    _allBookList.clear(); // Clear existing items in the list
    _allBookList.addAll(books.map((book) => Elibrary.fromJson(book)));
    }
}
